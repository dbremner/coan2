...
our @ISA=qw(Exporter);
our @EXPORT=qw(set_prog set_usage get_usage set_verbosity help usage_error progress activity outcome info warn error fatal bail cyg2win cygwin windows);
...
{
	my $msg = shift;
	if (PROGRESS >= $verbose_level) {
		printf STDOUT "$prog: progress: $msg",@_
	}
}
