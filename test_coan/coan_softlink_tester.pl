...
set_usage(
	"$prog: Exercise coan in traversing a directory tree containing softlinks " .
	"to verify that coan follows them correctly\n".
	"Usage:\n" .
...
run("$execdir/coan symbols --once-only --verbose --recurse --filter c,h  $rootname link2scrap link2options.h 2> $stderr_file");
progress("*** Done ***");
check_test_result();
