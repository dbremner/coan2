...
set_usage(
	"$prog: Exercise coan in traversing a directory tree. " .
	...
	"[--breadth BREADTH] [--depth DEPTH] [--seed SEED] [--keep] [--bail] " .
	"[--winexe]\n" .
	"$prog --help\n" .
	"Arguments:\n" .
...
$scrapdir = "$pkgdir/test_coan/scrap";
...
run_noerr("perl -Mstrict -I $pkgdir/perl -w $pkgdir/test_coan/scrap_dir_tree.pl --verbosity=fatal --wipe --seed=$seed --depth=$depth --rootname=$scrapdir " . 
...
# of .c and .h files
run("$execdir/coan symbols --once-only --verbose --recurse --filter cpp,h $arg_scrapdir $arg_scrapdir " . 
...
# Build list of -U-macros for all symbols found, max 64K in length.
open IN,"<$stdout_file" or die("Cannot open file \"$stdout_file\" for reading\n");
my $line;
while($line = <IN>) {
	chomp($line);
	$line =~ s/\: unconfigured//g;
	$undefs .= "-U$line ";
	last, if (length($undefs) >= (1 << 16));
}
close(IN);
if ($windows_exe && cygwin()) {
    $undefs =~ s/\r//g;
...
run("$execdir/coan source $undefs --verbose --recurse --filter cpp,h --backup \"~\" $arg_scrapdir 2> $stderr_file");
progress("*** Done ***");
check_test_result(2);
...
close(OUT);
my $cmd = "$execdir/coan source $undefs --verbose --recurse --filter cpp,h --backup \"~\" 2> $stderr_file < $infiles_file";
run("$execdir/coan source $undefs --verbose --recurse --filter cpp,h --backup \"~\" 2> $stderr_file < $infiles_file");
progress("*** Done ***");
check_test_result(3);
...
print OUT "$undefs\n";
close(OUT);
run("$execdir/coan source --file $undefs_file --verbose --recurse --filter cpp,h --backup \"~\" $arg_scrapdir 2> $stderr_file");
progress("*** Done ***");
check_test_result(4);
...
run("$execdir/coan symbols --once-only --verbose --keepgoing --recurse --filter cpp,h $arg_scrapdir 2> $stderr_file");
progress("*** Done ***");
...
sub check_test_result(@)
{
	...
	if ($stderr_text !~ m/info 0x02201: $infiles out of $infiles input files were reached/) {
		$fail = 1;
	}
...
sub tally_source_file()
{
	...
	++$infiles if ($file =~ m/\.cpp$/ or $file =~ m/\.h$/); 
}

sub gather_scrap_file()
{
    my $file = $File::Find::name;
    push(@scrap_files,"\"$file\"") if ($file =~ m/\.h$/ or $file =~ m/\.cpp$/); 
}
